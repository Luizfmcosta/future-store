...
export function BundleCard({ option, highlight }: { option: BundleOption; highlight?: boolean }) {
  ...
  const viewBundleLabel = getMessage(locale, "cart.viewBundlePart") ?? "View add-on";
  return (
    <Card className={`overflow-hidden p-0 ${highlight ? "ring-1 ring-stone-300/90" : ""}`}>
      <div className="flex gap-4 p-4">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-stone-500">Bundle</p>
          ...
          <div className="mt-2 flex flex-wrap items-baseline gap-2">
            ...
            <span className="text-[12px] text-stone-500">Save {formatBRL(option.savings)}</span>
          </div>
          <Link
